...
export class ParkingPage implements OnInit {
  ...
  getParkingDataList(export_to_excel?: boolean) {
    if (!export_to_excel){
    ...
    this.apiService.postFromServer(ApiPath.get_parking_data_and_total_number_by_sorting_and_limit_or_search, send_data, true).then((res: Response) => {
      ...
      let new_path = `/parking?page=${this.page}&limit=${this.limit}&sorting=${this.sorting}&direction=${this.direction}`;

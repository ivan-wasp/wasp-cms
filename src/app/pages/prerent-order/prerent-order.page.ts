...
import { PrerentOrderStatus } from 'src/app/schema';
...
@Component({
  selector: 'app-prerent-order',
  templateUrl: './prerent-order.page.html',
  styleUrls: ['./prerent-order.page.scss'],
})
export class PrerentOrderPage implements OnInit {
  sorting = null;
  direction = null;
  limit = null;
  page = null;
  ...
  getPrerentOrderDataList(export_to_excel?: boolean) {
    if (!export_to_excel){
    ...
    this.apiService.postFromServer(ApiPath.get_prerent_order_data_and_total_number_by_sorting_and_limit_or_search, send_data, true).then((res: Response) => {
      ...
      let new_path = `/prerent-order?page=${this.page}&limit=${this.limit}&sorting=${this.sorting}&direction=${this.direction}&status=${this.status}&user_id=${this.user_id||''}`;

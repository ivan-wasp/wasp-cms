...
@Component({
  selector: 'app-rbb-equipment',
  templateUrl: './rbb-equipment.page.html',
  styleUrls: ['./rbb-equipment.page.scss'],
})
export class RbbEquipmentPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getRbbEquipmentDataList();
  }

  getRbbEquipmentDataList() {
    ...
    this.apiService.postFromServer(ApiPath.get_rbb_equipment_data_and_total_number_by_sorting_and_limit_or_search, send_data, true).then((res: Response) => {
      let new_path = `/rbb-equipment?page=${this.page}&limit=${this.limit}&sorting=${this.sorting}&direction=${this.direction}`;

...
export class ViolationPage implements OnInit {
  ...
  async getUnpaidRanking(){
    const get_unpaid_ranking: Response = await this.dataService.getUnpaidViolationDataRanking();
  ...
  getViolationDataList(export_to_excel?: boolean) {
    if (!export_to_excel){
    ...
    this.apiService.postFromServer(ApiPath.get_violation_data_and_total_number_by_sorting_and_limit_or_search, send_data, true).then((res: Response) => {
      ...
      let new_path = `/violation?page=${this.page}&limit=${this.limit}&sorting=${this.sorting}&direction=${this.direction}&status=${this.status}&user_id=${this.user_id||''}&car_id=${this.car_id||''}`;

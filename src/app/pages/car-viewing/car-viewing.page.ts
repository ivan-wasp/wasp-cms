...
import { CarData, CarViewingStatus, CompensationPaymentStatus, UserData } from 'src/app/schema';
...
@Component({
  selector: 'app-car-viewing',
  templateUrl: './car-viewing.page.html',
  styleUrls: ['./car-viewing.page.scss'],
})
export class CarViewingPage implements OnInit {
  sorting = null;
  direction = null;
  limit = null;
  page = null;
  ...
  getCarViewingDataList(export_to_excel?: boolean) {
    if (!export_to_excel){
    ...
    this.apiService.postFromServer(ApiPath.get_car_viewing_data_and_total_number_by_sorting_and_limit_or_search, send_data, true).then((res: Response) => {
      ...
      let new_path = `/car-viewing?page=${this.page}&limit=${this.limit}&sorting=${this.sorting}&direction=${this.direction}&status=${this.status}&user_id=${this.user_id||''}&car_id=${this.car_id||''}`;
